...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    l = [restaurant_location(r) for r in cluster]
    return[mean([x[0] for x in l]), mean([x[1] for x in l])]
    # END Question 5
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        centroids = [find_centroid(clusters) for clusters in clusters]
        # END Question 6
        n += 1
    return centroids
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    return max([find_predictor(user, reviewed, fn) for fn in feature_fns], key = lambda x:x[1])[0]    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    for n in restaurants:
        if n in reviewed:
            rate_dic[n['name']] = user_rating(user, n['name'])
        else:
            rate_dic[n['name']] = predictor(n)  
